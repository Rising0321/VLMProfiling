...
class Linear(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.project = nn.Linear(embed_dim * 2, 1)

    def forward(self, image_latent):
        sv_pool = torch.mean(image_latent[:, :10, :], dim=1)
        st = image_latent[:, 10, :]
        temp = torch.cat([sv_pool, st], dim=1)
...
def main(args):
    ...
    for index in tqdm(ava_indexs):
        ...
        if not os.path.exists(sucess_path):
            continue
        _, images = get_images(index, city, args.model, model, preprocessor, image_dir)
        satellite = np.load(
